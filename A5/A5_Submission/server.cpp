...
private:
    ...
    const int HELO_LOG_SUPPRESS_SEC = 45; // suppress repeated HELO logs for 45 seconds
    ...
    void handleNewPeerConnection(int sockfd, const std::string& ip, int port, std::vector<pollfd>& fds, const std::string& expected_peer_name = "", int requester_fd = -1) {
        ...
        bool sent_ok = Networking::sendMessage(sockfd, helo);
        ...
        if (sent_ok) {
            ...
            log("WARNING: Failed to send HELO to peer (" + ip + ":" + std::to_string(port) + ") fd=" + std::to_string(sockfd));
        }
    ...
    // ### ATTEMPT AT FIXING THE 100% CPU BUG: #### //
    
    // Buffered receive: accumulates partial frames in ConnectionState.buffer
    // Returns: complete message string, or empty string if no complete message yet
    // Returns "__FRAME_CORRUPTION__" if framing is invalid
    // Returns "__CONNECTION_CLOSED__" if peer closed connection (EOF)
    std::string bufferedReceive(int sockfd) {
        if (connection_states.find(sockfd) == connection_states.end()) {
        ...
        char tmp[4096];
        ssize_t nread = recv(sockfd, tmp, sizeof(tmp), 0);
        
        if (nread == 0) {
            // EOF - peer closed connection
            return "__CONNECTION_CLOSED__";
        }
        
        if (nread < 0) {
            if (errno == EAGAIN || errno == EWOULDBLOCK) {
                // No data available right now - check if we have a complete message in buffer
            ...
            state.buffer.append(tmp, nread);
        }
        
        // Try to parse a complete message from buffer
        // Minimum frame: SOH(1) + length(2) + STX(1) + ETX(1) = 5 bytes
        if (state.buffer.size() < 5) {
            return "";  // Not enough data yet
        }
        
        // Check SOH
        if (static_cast<uint8_t>(state.buffer[0]) != SOH) {
            // Frame corruption
            return "__FRAME_CORRUPTION__";
        }
        
        // Check if we have at least 3 bytes for header (SOH + 2-byte length)
        if (state.buffer.size() < 3) {
            return "";  // Wait for more data
        }
        
        // Extract length from header
        uint16_t totalLength = (static_cast<uint8_t>(state.buffer[1]) << 8) | static_cast<uint8_t>(state.buffer[2]);
        ...
        std::vector<uint8_t> frame_bytes(state.buffer.begin(), state.buffer.begin() + totalLength);
        
        // Remove this frame from buffer
        state.buffer.erase(0, totalLength);
        
        // Decode the message
        try {
            return Protocol::decodeMessage(frame_bytes);
        } catch (...) {
            return "__FRAME_CORRUPTION__";
        }
    }
...
public:
    ...
    void start() {
        ...
        try {
            ...
            // Main event loop
            while (running) {
                int poll_count = poll(fds.data(), fds.size(), 500); // 500ms timeout
                ...
                for (size_t i = 0; i < fds.size(); i++) {
                    ...
                    // IMPORTANT: For pending outgoing connections, check POLLOUT first
                    // A socket can have both POLLOUT and POLLHUP/POLLERR set when connect() completes
                    // We need to check SO_ERROR to determine if connection succeeded before treating as error
                    bool is_pending_connection = false;
                    for (const auto& pc : pending_connections) {
                        if (pc.sockfd == fd) {
                    ...
                    // Check for errors (but skip if this is a pending connection with POLLOUT set)
                    // ALSO: If POLLIN is set along with POLLHUP, read the data first before closing
                    if ((fds[i].revents & (POLLHUP | POLLERR | POLLNVAL)) && 
                        !(is_pending_connection && (fds[i].revents & POLLOUT)) &&
                        !(fds[i].revents & POLLIN)) {  // Don't close if there's data to read
                        if (fd != server_fd) {
                    ...
                    // Data available on existing connection
                        ...
                        // Try to read and process messages in a loop until no complete message is available
                        // This prevents busy-waiting when partial frames are buffered
                        bool should_continue = true;
                        int message_count = 0;
                        const int MAX_MESSAGES_PER_POLL = 10;  // Prevent one fd from starving others
                        
                        while (should_continue && message_count < MAX_MESSAGES_PER_POLL) {
                            std::string message = bufferedReceive(fd);
                            
                            // Check for connection closed
                            if (message == "__CONNECTION_CLOSED__") {
                                log("Connection fd=" + std::to_string(fd) + " closed by peer");
                                closeConnection(fd, fds);
                                i--;  // Adjust index after removal
                            ...
                            // Check for frame corruption signal
                            else if (message == "__FRAME_CORRUPTION__") {
                                ...
                                if (connection_states.find(fd) == connection_states.end()) {
                                    i--;  // Adjust index after removal
                                    should_continue = false;
                                    break;
                                }
                                // Continue loop to check if more complete messages are buffered
                            }
                            else {
                                // Empty message means:
                                // - No complete frame available yet (partial read buffered)
                                // - No more data available (EAGAIN)
                                // Stop trying to read from this fd until next poll event
                                should_continue = false;
                            }
                        }
                    }
